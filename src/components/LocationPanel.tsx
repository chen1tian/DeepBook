...
export default function LocationPanel({ open, onClose, value }: Props) {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      >
        <div className="flex h-11 shrink-0 items-center justify-between border-b border-white/5 px-3">
          <span className="text-xs font-medium text-zinc-400">当前地点</span>
        ...
        <div className="flex flex-1 items-center justify-center p-6">
          {value ? (
            <div className="text-center">
              <MapPin size={32} className="mx-auto mb-3 text-emerald-400" />
              <p className="text-lg font-medium text-zinc-200">{value}</p>
            </div>
          ) : (
            <p className="text-sm text-zinc-600">暂无地点数据</p>
          )}
        </div>
      </div>
    </div>
