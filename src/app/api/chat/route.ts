...
const GET_SETTINGS_TOOL = {
  ...
  function: {
    name: "get_settings",
    description: "查看当前故事的所有世界设定。用户问「有哪些设定」「世界观设定」「看看设定」时调用。需要 bookId。",
    parameters: {
      type: "object" as const,
      properties: {},
    },
...
const ADD_SETTING_TOOL = {
  type: "function" as const,
  function: {
    name: "add_setting",
    description: "添加一条新的世界设定。用户说「添加一个设定」「新增世界观规则」「记下这个设定」时调用。需要 bookId。",
    parameters: {
      type: "object" as const,
      properties: {
        key: { type: "string", description: "设定名称（简短概括）" },
        value: { type: "string", description: "设定内容（详细描述）" },
        category: { type: "string", description: "分类：世界观/人物关系/历史事件/规则体系/其他" },
      },
      required: ["key", "value"],
    },
  },
...
const UPDATE_SETTING_TOOL = {
  type: "function" as const,
  function: {
    name: "update_setting",
    description: "修改一条已有的世界设定。用户说「修改××设定」「更新世界观」「改一下这个设定」时调用。需要 bookId。",
    parameters: {
      type: "object" as const,
      properties: {
        key: { type: "string", description: "要修改的设定名称（用来查找）" },
        newKey: { type: "string", description: "新的设定名称（可选，不修改名称则不传）" },
        value: { type: "string", description: "新的设定内容（可选）" },
...
const DELETE_SETTING_TOOL = {
  type: "function" as const,
  function: {
    name: "delete_setting",
    description: "删除一条世界设定。用户说「删除××设定」「去掉这个设定」时调用。⚠️ 必须先向用户确认后再调用此工具。需要 bookId。",
    parameters: {
      type: "object" as const,
      properties: {
        key: { type: "string", description: "要删除的设定名称" },
      },
      required: ["key"],
    },
  },
...
- 管理世界设定：当用户问「有哪些设定」「世界观设定」时，用 get_settings 查看。当用户说「添加/记下一个设定」时，用 add_setting 添加。当用户说「修改/更新××设定」时，用 update_setting 修改。当用户说「删除/去掉××设定」时，先确认，再调用 delete_setting 删除
...
function buildTools(task: string | null, bookId?: number) {
  ...
  if (bookId) {
    list.push(GET_DIALOGUE_OVERVIEW_TOOL, GET_DIALOGUE_MESSAGES_TOOL, UPDATE_DIALOGUE_CONFIG_TOOL, GET_PLOT_STATE_TOOL, GET_PLOT_PROMPTS_TOOL, GET_SETTINGS_TOOL, ADD_SETTING_TOOL, UPDATE_SETTING_TOOL, DELETE_SETTING_TOOL);
  }
...
注意：只返回 JSON，不要其他文字。节点按故事发展顺序排列。`;
    ...
    case "get_settings": {
      ...
      const { getStoryState: getSS } = await import("@/lib/story-state");
      const ss = getSS(book.active_dialogue_id);
      const allSettings = ss.settings || [];
    ...
    case "add_setting": {
      ...
      const { getStoryState: getSS2, saveStoryState: saveSS } = await import("@/lib/story-state");
      const ss2 = getSS2(book.active_dialogue_id);
      const allSettings = ss2.settings || [];

      const newKey = (args.key as string).trim();
      if (!newKey) return { success: false, error: "设定名称不能为空" };

      // Check for duplicate key
      if (allSettings.some((s) => s.key === newKey)) {
        return { success: false, error: `设定「${newKey}」已存在，请用 update_setting 修改` };
      }
      ...
      ss2.settings = allSettings;
      saveSS(book.active_dialogue_id, ss2);

      return { success: true, message: `世界设定「${newKey}」已添加`, setting: newSetting };
    }
    case "update_setting": {
      if (!bookId) return { success: false, error: "需要先打开一个故事" };
      const book = await getBook(bookId, userId);
      ...
      const { getStoryState: getSS3, saveStoryState: saveSS2 } = await import("@/lib/story-state");
      const ss3 = getSS3(book.active_dialogue_id);
      const allSettings = ss3.settings || [];

      const targetKey = (args.key as string).trim();
      const idx = allSettings.findIndex((s) => s.key === targetKey);
      if (idx === -1) return { success: false, error: `未找到设定「${targetKey}」` };

      if (args.newKey) allSettings[idx].key = (args.newKey as string).trim();
      if (args.value !== undefined) allSettings[idx].value = args.value as string;
      if (args.category !== undefined) allSettings[idx].category = args.category as string;

      ss3.settings = allSettings;
      saveSS2(book.active_dialogue_id, ss3);

      return { success: true, message: `设定已更新`, setting: allSettings[idx] };
    }
    case "delete_setting": {
      if (!bookId) return { success: false, error: "需要先打开一个故事" };
      const book = await getBook(bookId, userId);
      ...
      const { getStoryState: getSS4, saveStoryState: saveSS3 } = await import("@/lib/story-state");
      const ss4 = getSS4(book.active_dialogue_id);
      const allSettings = ss4.settings || [];

      const targetKey = (args.key as string).trim();
      const idx = allSettings.findIndex((s) => s.key === targetKey);
      if (idx === -1) return { success: false, error: `未找到设定「${targetKey}」` };

      allSettings.splice(idx, 1);
      ss4.settings = allSettings;
      saveSS3(book.active_dialogue_id, ss4);

      return { success: true, message: `世界设定「${targetKey}」已删除` };
    }
    default:
